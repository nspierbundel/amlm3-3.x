...
//	}
	...
	if (is_on) {
		if (clk_32k_on == 0) {			
            ...
            aml_set_reg32_mask(P_HHI_GEN_CLK_CNTL, 7<<9);//set clk source
            aml_clr_reg32_mask(P_HHI_GEN_CLK_CNTL, 0x3f<<0);//set div ==1
            aml_set_reg32_mask(P_HHI_GEN_CLK_CNTL, 1<<8);//set enable clk
            switch(wifi_info.clock_32k_pin){
                case 175 : /* GPIOAO_11 */
                    aml_set_reg32_mask(P_AO_RTI_PIN_MUX_REG, 1<<21);
                    break;
                case 102 : /* GPIOC_15 */
                    aml_set_reg32_mask(P_PERIPHS_PIN_MUX_3, 1<<22);
                    break;
                case 22 : /* GPIOY_22 */
                    aml_set_reg32_mask(P_PERIPHS_PIN_MUX_3, 1<<21);
                    break;
                default :
                    printk("%s: not support pin name from dts file.\n", __func__);
                    break;
                }
#else
            printk("%s : No support CPU type!!!\n", __func__);
#endif
			udelay(200);
		}
		++clk_32k_on;
...
#elif (defined CONFIG_ARCH_MESON3)
                        aml_clr_reg32_mask(P_HHI_GEN_CLK_CNTL, 1<<8);//disable clk gate
#else
                        printk("%s : No support CPU type!!!\n", __func__);
#endif

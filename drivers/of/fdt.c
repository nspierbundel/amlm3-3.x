				     ...
				     int depth, void *data)
{
	...
	__be32 * endp;
#endif
        unsigned int firmware_addr=0x9ff00000;
	...
	if(aml_reserved_end+1 > firmware_addr)
	{
		...
		return -1;
            ...
            if(total == 0x10000000)  // 256M
                firmware_addr = 0x8ff00000;
            early_init_dt_add_memory_arch(aml_reserved_end+1,firmware_addr-aml_reserved_end-1);  //1M reserved for firmware
            if((phys_offset+total-(firmware_addr+0x100000)-high_reserve_size)>0)
                early_init_dt_add_memory_arch(firmware_addr+0x100000, phys_offset+total-(firmware_addr+0x100000)-high_reserve_size);
                printk("reserved 0x%08x-0x%08x 1M memory for firmware\n", firmware_addr-0x10000, firmware_addr);
	}
#endif

...
int audio_clock_config_table[][11][2]=
{
  ...
  //
	...
	//24M
        ...
        {(128<<0) | (5<<9) | (1<<14) , (75-1)}, // 8K, 1.024M*4=4.096M
        {(147<<0) | (5<<9) | (0<<14) , (125-1)}, //11.025K,1.4112*4=5.6448M
        {(32<<0) | (1<<9) | (1<<14) , (250-1)}, // 12K, 1.536M
        {(128<<0) | (5<<9) | (0<<14) , (75-1)}, // 16K, 2.048M*4=8.192M
        {(143<<0) | (8<<9) | (1<<14) , (19-1)}, //22.050K, 2.8224*4=11.2896M
...
void audio_set_aiubuf(u32 addr, u32 size, unsigned int channel)
{
    ...
    WRITE_MPEG_REG(AIU_MEM_I2S_END_PTR, (addr & 0xffffffc0) + (size & 0xffffffc0) - 256);   //this is for 16bit 2 channel
	...
	WRITE_MPEG_REG(AIU_I2S_MISC,		0x0018 );	// Release hold and force audio data to left or right
	
	if((channel==8) || READ_MPEG_REG_BITS(AIU_I2S_SOURCE_DESC,11,1)){
...
void audio_set_958outbuf(u32 addr, u32 size,int flag)
{
	...
	if(READ_MPEG_REG(AIU_MEM_IEC958_START_PTR) == READ_MPEG_REG(AIU_MEM_I2S_START_PTR)){
        ...
        if(flag == 0){
          WRITE_MPEG_REG(AIU_MEM_IEC958_END_PTR, (addr & 0xffffffc0) + (size & 0xffffffc0) - 256);    // this is for 16bit 2 channel
        }else{
    ...
    if (mode < sizeof(mask)/ sizeof(unsigned short)) {
       /* four two channels stream */
        ...
        if (mode == AIU_I2S_MODE_PCM16) {
			...
			WRITE_MPEG_REG_BITS(AIU_I2S_SOURCE_DESC, 1, 11, 1);
			WRITE_MPEG_REG_BITS(AIU_I2S_SOURCE_DESC, 1, 4, 1);

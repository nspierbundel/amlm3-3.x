/*
 * aml_m3_asoc_audio.c  --  SoC audio for AML M3
 *
...
{
	int val = amlogic_get_value(p_aml_audio->gpio_hp_det,"aml_m3");
...
#if HP_IRQ	
	p_aml_audio->gpio_hp_det = of_get_named_gpio(card->dev->of_node,"aml_m3_gpio",0);
...
static void aml_m3_pinmux_init(struct snd_soc_card *card)
{
	struct aml_audio_private_data *p_aml_audio;
	const char *str;
	int ret;
	p_aml_audio = snd_soc_card_get_drvdata(card);
	p_aml_audio->pin_ctl = devm_pinctrl_get_select(card->dev, "aml_m3_audio");
#if HP_DET
	ret = of_property_read_string(card->dev->of_node, "aml_m3_gpio", &str);
	if (ret) {
		printk("aml_m3: faild to get gpio!\n");
	}
...
//	p_aml_audio->gpio_hp_det = of_get_named_gpio(card->dev->of_node,"rt5631_gpio",0);
	...
	amlogic_gpio_request_one(p_aml_audio->gpio_hp_det,GPIOF_IN,"aml_m3");
#endif

	ret = of_property_read_string(card->dev->of_node, "mute_gpio", &str);
	if (ret < 0) {
		printk("aml_snd_m3: faild to get mute_gpio!\n");
	}else{
	...
	printk("=%s==,aml_m3_pinmux_init done,---%d\n",__func__,p_aml_audio->det_pol_inv);
}

static void aml_m3_pinmux_deinit(struct snd_soc_card *card)
{
	struct aml_audio_private_data *p_aml_audio;

	p_aml_audio = snd_soc_card_get_drvdata(card);

	amlogic_gpio_free(p_aml_audio->gpio_hp_det,"aml_m3");
...
static const struct of_device_id amlogic_audio_dt_match[]={
	{	.compatible = "sound_card,aml_m3",//"amlogic,aml_rt5631_audio",
	},
	{},
};
#else
...
static int __init aml_m3_audio_init(void)
{
    return platform_driver_register(&aml_m3_audio_driver);
}

static void __exit aml_m3_audio_exit(void)
{
    platform_driver_unregister(&aml_m3_audio_driver);
}

#ifdef CONFIG_DEFERRED_MODULE_INIT
deferred_module_init(aml_m3_audio_init);
#else
module_init(aml_m3_audio_init);
#endif
module_exit(aml_m3_audio_exit);

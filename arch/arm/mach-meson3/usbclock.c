 ...
 *	by Victor Wan 2011.12.5 @Shanghai
 *
...
int set_usb_phy_clk(int port_idx,int is_enable)
{
    ...
    if(port_idx==0)
		...
		if(no_clock_inited){
			...
			config.d32 = aml_read_reg32(p_usb_reg);
			printk("USB remove clock source: %s\n",clock_src_name[config.b.clk_sel]);
			config.b.clk_en = 0;
			aml_write_reg32(p_usb_reg,config.d32);
		}	
	}
...
int set_usb_phy_id_mode(int port_idx,unsigned int phy_id_mode)
{
